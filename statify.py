...
with open('kill_stats.js', 'w') as outfile:
    with open('kill_stats.csv', newline='') as f:
        ...
        for wins, losses, us_kills, them_kills in csv.reader(f):
            print('"[%d,%d]" : [%d,%d],' % tuple(int(x) for x in (us_kills, them_kills, wins, losses)), file=outfile)
        print('};', file=outfile);

with open('tower_stats.js', 'w') as outfile:
    with open('tower_stats.csv', newline='') as f:
        print('var tower_stats = {', file=outfile);
        for wins, losses, us_inhibs, us_towers, them_inhibs, them_towers in csv.reader(f):
...
with open('joint_stats.js', 'w') as outfile:
    with open('joint_stats.csv', newline='') as f:
        ...
        for wins, losses, us_inhibs, us_towers, us_kills, them_inhibs, them_towers, them_kills in csv.reader(f):
